...
% load sea2; name = 'Ense+Sea'; nFolds=200;
% ninput=3;
% noutput=2;
% sourcedata=data;


% load electricitypricing; ninput=8; name = 'Ense+Elec'; nFolds=91;
% noutput=2;
% sourcedata=data;
...
% load susytraintest; name = 'Ense+Susy'; nFolds=10000;
% ninput=18;
% noutput=2;
% sourcedata=data;

%
% load datahyp; name = 'Ense+Hyp'; nFolds=240;
% ninput=4;
% noutput=2;
% sourcedata=data;
%
% load Hepmass2m; name = 'Ense+Hepm'; nFolds=4000;
% ninput=28;
% noutput=2;
% sourcedata=data;
%
% load kdd10persenshuff3; name = 'Ense+Kdd'; nFolds=1000;
% ninput=41;
% noutput=2;
% sourcedata=data;
%

% load weather; name = 'Ense+Weather'; nFolds=36;
% ninput=8;
% noutput=2;
% sourcedata=data;

% load rfid; name = 'Ense+Rfid'; nFolds=560;
% ninput=3;
% noutput=4;
% sourcedata=data;


% load rlcps; name = 'Ense+Rlcps'; nFolds=10000;
% ninput=9;
% noutput=2;
% sourcedata=data;
        ...
        for k1=1:r
            ...
            for m=1:ensemble
                ...
                for i=1:size(network(m).Center,1)
                    ...
                    populationclassoverall(:,:,end+i)=network(m).population_class_cluster(:,:,i);
                end
            ...
            if local==1
                ...
                for k2=1:totalrule
                    ...
                    volume(k2)=det(spreadoverall(:,:,k2));
            ...
            if (outputprobability>thres || max(inputprobability)>thres) && k1~=r && count_samples>5
                ...
                if mod(k1,p)==0
                    ...
                    covariance_old=covariance;
                    if ensemblepruning2==1 && ensemble>1 && isempty(pruning_list) && activation==0 && k1==r
                    ...
                    if k1==r
                        ...
                        elseif (mean(abs(Xstat-Ystat)))>=mean(Ybound1) && (mean(abs(Xstat-Ystat)))<mean(Ybound)
                            ...
                            type_feature_weighting=7;
                            buffer=[];
                            [Weight,Center,Spread,rule,y,error,rules_significance,rules_novelty,datum_novelty,input_significance,population,time,classification_rate_testing,normalized_out,feature_weights,population_class_cluster,focalpoints,sigmapoints]=pclass_multivariate_update1(Selectedsamples,fix_the_model,paramet,demo,ninput,mode,drift,type_feature_weighting,network(index1));
                            [v,vv]=size(Center);
                            network_parameters=v*subset+(subset)^(2)*v+(subset+1)*v*noutput;
...
SizeNetrat=mean(E);
SizeNetdev=std(E);
CountSampleRat=mean(F);
CountSampleRatDev=std(F);
training_time = [mean(update_time) std(update_time)];
testing_time = [mean(test_time) std(test_time)];

ass2=sprintf('results_plus/%sresults.mat',name);
save(ass2,'Clastestrat','Clastestdev','TotRulerat','TotRuledev','TotPamrat','TotPamdev','SizeNetrat','SizeNetdev','training_time','testing_time','CountSampleRat','CountSampleRatDev','nFolds','chunk_size','Residual_error');