...
[totalrule,ndimension]=size(centeroverall);
   ...
   di=zeros(totalrule,1);
      ...
      %  dis1=dis./spreadoverall(k2,:);
        ...
        volume(k2)=abs(det(spreadoverall(:,:,k2)));
     ...
     if  k1==size(Data,1)
      ...
      network(m).ensemble_weight=network(m).ensemble_weight/a;
        ...
        activation=1;
         ...
         %{
             for iter=1:noutput
                        covariance(:,:,iter)=(covariance_old(:,:,iter)*(k-1)+(((k-1)/k)*outputcovar(:,:,iter)))/k;
             end
             %}
             covariance_old=covariance;
          ...
          merged_list(end+1,2)=hh;
           ...
           break
                ...
                %{
                          ...
                          del=find(merged_list(:,1)>ensemble);
...
buffer=Selectedsamples;
 fix_the_model=size(Selectedsamples,1);
    ...
    buffer=[buffer;Selectedsamples];
     fix_the_model=size(Selectedsamples,1)+size(buffer,1);
end
