...
[totalrule,ndimension]=size(centeroverall);
   ...
   if k1==1000
       Zstat=mean(Data(1:1000,1:ninput));
    cuttingpoint=0;
        for cut=1:1000
...
Ystat=Zstat;  
 ...
 if (outputprobability>thres || max(inputprobability)>thres) && k1~=1000 && count_samples>5
      % thres=thres*(1-s);
     ...
     if  k1==1000
        ...
        activation=1;
            ...
            %outputmean(iter1,iter)=mean(storeoutput(:,iter1,iter));
             ...
             %           standarized_mean=standarized_mean_old+(z_current-standarized_mean_old)/counter;
                 merged_list=[];
 
             covariance_old=covariance;
        if ensemblepruning2==1 && ensemble>1 && isempty(pruning_list) && activation==0 && k1==1000
