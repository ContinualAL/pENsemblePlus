...
Ystat=Zstat;  
   ...
   if (outputprobability>thres || max(inputprobability)>thres) && k1~=1000 && count_samples>5
    ...
    [maxout1,trueclasslabel]=max(stream(ndimension+1:end)); 
       ...
       %if thres*(1+s)<1 || count_samples<=5
       %  thres=thres*(1+s);
       %end
        ...
        activation=1;
            ...
            if k1==1000
                 outputcovar=zeros(ensemble,ensemble,noutput);
                  outputvar=zeros(ensemble,noutput);
                        for iter=1:ensemble
            ...
            %outputmean(iter1,iter)=mean(storeoutput(:,iter1,iter));
             %               end
                        for iter1=1:ensemble
                            ...
                            outputvar(iter1,iter2)=var(storeoutput(:,iter2,iter1));
             ...
             covariance_old=covariance;
                     if ensemblepruning2==1 && ensemble>1 && activation==0 && k1==1000
             merged_list=[];
